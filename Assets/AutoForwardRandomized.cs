...
public class AutoForwardRandomized : MonoBehaviour
{
    private bool isAutoForward = false;
    private PA_DroneAxisInput droneInput;
    private PA_DroneController droneController;
    private bool randFlag = false; //keep track if random value happend for ErrP
    ...
    private IEnumerator HandleTriggeredStrafe()
    {
        ...
        if(Random.value <= 0.3) { direction *= 0; randFlag = true; } //adding random strafe for ErrP
